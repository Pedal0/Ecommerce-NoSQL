...
app.put('/users/me', authenticate, async (req, res) => {
    try {
        ...
        await addLog(req.user.id, 'UPDATE_USER', `Utilisateur ${user.username} mis à jour`);
        res.json(user);
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
...
app.post('/products', authenticate, checkRole(['vendeur', 'admin']), async (req, res) => {
    try {
        const product = new Product({ ...req.body, seller: req.user.id });
        await product.save();
        await addLog(req.user.id, 'CREATE_PRODUCT', `Produit ${product.name} créé`);
...
app.put('/products/:id', authenticate, checkRole(['vendeur', 'admin']), async (req, res) => {
    try {
        const product = await Product.findByIdAndUpdate(req.params.id, req.body, { new: true });
        await addLog(req.user.id, 'UPDATE_PRODUCT', `Produit ${product.name} mis à jour`);
        res.json(product);
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
...
app.delete('/products/:id', authenticate, checkRole(['vendeur', 'admin']), async (req, res) => {
    try {
        await Product.findByIdAndDelete(req.params.id);
        await addLog(req.user.id, 'DELETE_PRODUCT', `Produit supprimé, ID: ${req.params.id}`);
...
app.post('/orders', authenticate, checkRole(['client']), async (req, res) => {
    try {
        ...
        await order.save();
        await addLog(req.user.id, 'CREATE_ORDER', `Commande créée avec total ${total}`);
...
app.put('/orders/:id/status', authenticate, checkRole(['gestionnaire', 'admin']), async (req, res) => {
    try {
        const { status } = req.body;
        const order = await Order.findByIdAndUpdate(req.params.id, { status }, { new: true });
        await addLog(req.user.id, 'UPDATE_ORDER', `Commande ${order._id} mise à jour au statut ${status}`);
        res.json(order);
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
...
app.delete('/orders/:id', authenticate, checkRole(['admin']), async (req, res) => {
    try {
        await Order.findByIdAndDelete(req.params.id);
        await addLog(req.user.id, 'DELETE_ORDER', `Commande supprimée, ID: ${req.params.id}`);
